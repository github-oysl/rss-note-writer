...
class Scheduler:
    """
    定时调度器：负责定时从 RSS 源获取链接并调用 API 写入笔记。
    """
    ...
    def __init__(self, delay_seconds: int = 10):
        ...
        :param delay_seconds: 每次 API 调用之间的延迟时间（秒），默认 10 秒
        """
    ...
    def run(self, configs: List[Dict], token: str) -> None:
        ...
        :param configs: RSS 配置列表，每个配置包含 rss_url、topic_id、topic_directory_id
        :param token: Bearer token 用于 API 认证
        """
        ...
        for config_index, config in enumerate(configs):
            ...
            try:
                ...
                links = self.rss_fetcher.fetch_links(rss_url, max_links=10)
                ...
                for link in links:
                    ...
                    try:
                        ...
                        if response.ok:
                            ...
                            self.logger.warning(f"添加链接失败，状态码: {response.status_code}, 链接: {link}")
